...
class BrodanBackupTests(unittest.TestCase):
    ...
    def test_backup_ui_is_owner_only(self):
        ...
        self.assertIn("BRODAN: host cmd", mod.LEFTOVER_BACKUP_ACTION_NAMES)
        ...
        self.assertIn("FILESTORE_ACTION_NAME", installer)
        self.assertIn("--filestore-now", installer)
        self.assertIn("بدون المرفقات", installer)

    def test_filestore_stream_is_complete_tar_to_onedrive(self):
        script = mod.filestore_stream_script()
        compile(script, "brodan_filestore_stream.py", "exec")
        self.assertIn("tarfile", script)
        self.assertIn("mtime=0", script)
        self.assertIn("rcat", script)
        self.assertIn("--size", script)
        self.assertIn("brodan_backup.lock", script)
        self.assertIn("xmlrpc", script)
        self.assertIn("127.0.0.1:8069", script)
        self.assertIn("RESTORE_BRODANSH.txt", script)
        self.assertIn("brodansh/", script)
        write_py = mod.filestore_stream_write_program()
        self.assertIn("brodan_filestore_stream.py", write_py)
        cmd = mod.filestore_rcat_program("brodansh", "Brodansh_Backups", "brodansh_filestore_1.tar.gz")
        self.assertIn("brodan_filestore_stream.py", cmd)
        self.assertIn("brodansh_filestore_1.tar.gz", cmd)
        self.assertIn("nohup python3", cmd)
        self.assertEqual(mod.filestore_rcat_program("", "Brodansh_Backups", "f"), "")
        self.assertEqual(mod.FILESTORE_ACTION_NAME, "BRODAN: تشغيل نسخة المرفقات")
        self.assertEqual(mod.RPC_TMP_PARAM, "brodan.rpc_tmp")
